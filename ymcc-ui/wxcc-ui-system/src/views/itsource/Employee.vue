<template>
	<section>
		...
		<el-table :data="tableData" highlight-current-row v-loading="listLoading" @selection-change="selsChange" style="width: 100%;">
			...
			<el-table-column prop="inputTime" label="注册时间" >
			</el-table-column>
		...
		<el-col :span="24" class="toolbar">
			...
			<el-pagination layout="prev, pager, next" @current-change="selectPage" :page-size="pageSize" :current-page="pageNo" :total="total"  style="float:right;">
			</el-pagination>
		</el-col>
          ...
          pageNo: this.pageNo,
			...
			del: function (row) {
				this.$confirm('确认删除该记录吗?', '提示', { type: 'warning' }).then(() => {
					this.listLoading = true;
					this.$http.delete("/system/systemdictionaryitem/delete/"+row.id).then(result=>{
						let {success , data, message ,code} = result.data;
						if(success){
